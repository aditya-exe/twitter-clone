  ...
  return (
    <div className={"hidden sm:flex flex-col p-2 xl:items-start fixed h-full xl:ml-24"}>
      ...
      {data ? (
        <>
          <button
            className={"bg-blue-400 rounded-full text-white w-56 h-12 font-bold shadow-md hover:brightness-95 text-lg hidden xl:inline"}>
            Tweet
          </button>

          <div className={"hoverEffect text-gray-700 flex items-center justify-center xl:justify-start mt-auto"}>
            <img onClick={() => signOut()} src={data.user.image} alt={"user-image"}
        ...
        <button onClick={() => signIn()}
                className="bg-blue-400 rounded-full text-white w-36 h-12 font-bold shadow-md hover:brightness-95 text-lg hidden xl:inline">Sign
          In</button>
      )}
