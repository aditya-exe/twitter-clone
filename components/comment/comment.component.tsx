...
const Comment: React.FC<CommentProps> = ({ commentId, orignalPostId, comment }) => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      collection(db, "posts", orignalPostId, "comments", commentId, "likes"),
      (snapshot) => setLikes(snapshot.docs)
    )
  }, [orignalPostId, commentId])
  ...
  const likeComment = async () => {
    if (session) {
      if (hasLiked) {
        await deleteDoc(doc(db, "posts", orignalPostId, "comments", commentId, "likes", session?.user.uid))
      } else {
  ...
  const deleteComment = async () => {
    if (window.confirm("Are you sure you want to delete this comment?")) {
      deleteDoc(doc(db, "posts", orignalPostId, "comments", commentId));
    }
  }
