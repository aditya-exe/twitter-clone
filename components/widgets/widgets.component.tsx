...
const Widgets: React.FC<WidgetsProps> = ({newsResults, randomUsers}) => {
  ...
  const [userNum, setUserNum] = useState<number>(3)
      ...
      <div className="text-gray-700 space-y-3 bg-gray-100 rounded-xl pt-2 w-[90%] xl:w-[75%]">
        ...
        <AnimatePresence>
          {newsResults.slice(0, articleNum).map((article: { title: string }) => (
            <motion.div key={article.title} initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}}
        ...
        <button className="text-blue-300 pl-4 pb-3 hover:text-blue-400"
                onClick={() => setArticleNum(articleNum + 3)}>Show more
        </button>
      </div>
      ...
      <div className="sticky top-16 text-gray-700 space-y-3 bg-gray-100 rounded-xl pt-2 w-[90%] xl:w-[75%]">
        ...
        <AnimatePresence>
          {randomUsers.slice(0, userNum).map((user: { login: { username: string }; picture: { thumbnail: string }; name: { first: string; last: string } }) => (
            <motion.div key={user.login.username} initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}}
                        transition={{duration: 1}}>
              <div key={user.login.username}
                ...
                <button className="ml-auto bg-black text-white rounded-full text-sm px-3.5 py-1.5 font-bold">Follow
                </button>
              </div>
            </motion.div>
          ))}
        </AnimatePresence>
        <button className="text-blue-300 pl-4 pb-3 hover:text-blue-400" onClick={() => setUserNum(userNum + 3)}>Show
          more
        </button>
      </div>
